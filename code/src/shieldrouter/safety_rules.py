...
def assess_safety(row: dict[str, str], business: dict[str, str] | None = None) -> SafetyAssessment:
    ...
    if CREDENTIAL_RE.search(low) and not advisory_context:
        signals.append("credential_or_login_code_request")
    if CREDENTIAL_RE.search(low) and REQUEST_RE.search(low) and not advisory_context:
    ...
    if business:
        ...
        if not verified and any(word in low for word in ("bank", "account", "card", "payment", "refund", "loan", "kyc")):
    ...
    if signals:
        if "chain_or_excessive_forwarding" in signals:
            msg_type = "forward"
        elif signals == ["sender_report_history"] or "sender_report_history" in signals and not low:
            msg_type = "spam"
        else:
            msg_type = "scam"
